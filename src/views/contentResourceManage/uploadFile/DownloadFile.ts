 ...
 * @LastEditTime: 2021-06-09 15:54:24
...
export class DownloadFile {
  ...
  // static getFileUrl(p9FileIdx: any) {
  //   // 获取文件存放服务器IP和路径
  //   this.getFileCluster().then((res) => {
  //     if (res.result === "SUCCESS") {
  //       const filePathData = res.data;
  //       // 文件由p9转移至p2s
  //       this.fetchFile(filePathData.host, filePathData.tempPath, p9FileIdx).then((tempFile) => {
  //         if (tempFile.length) {
  //           const path = tempFile[0];
  //           const name = tempFile[1];
  //           // 直接下载或者返回下载路径
  //           // window.open(`http://${filePathData.host}/ecpweb/getLocalFile.action?relativePath=${path}&fileName=${name}`);
  //           return `http://${filePathData.host}/ecpweb/getLocalFile.action?relativePath=${path}&fileName=${name}`;
  //         }
  //       });
  //     }
  //   });
